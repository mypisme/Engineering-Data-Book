...
BOOK = ROOT / "translations" / "book-ii"
...
def check_inline_math(errors: list[str]) -> None:
    for md in sorted(BOOK.glob("ch*/*.md")) + sorted(GLOSSARY.glob("*.md")):
...
def check_chapter_assets(errors: list[str], warnings: list[str]) -> None:
    page_ranges = {
        "ch01-basic-heat-transfer": (8, 58),
        "ch02-sensible-heat-transfer": (59, 143),
        "ch03-condensing-heat-transfer": (144, 209),
        "ch04-air-cool-heat-exchangers": (210, 241),
        "ch05-boiling-heat-transfer": (242, 305),
    }
    for chapter in sorted(BOOK.glob("ch*")):
        ...
        expected = page_ranges.get(chapter.name)
        if expected:
...
def main() -> int:
    ...
    if errors:
        ...
        for error in errors:
            print(f"- {error}")
        return 1
    print("Book II publication verification passed")
    return 0
